...
app5 = dash.Dash(__name__, title="Dashboard",external_scripts=external_scripts,requests_pathname_prefix='/app5/',external_stylesheets=[dbc.themes.SLATE])
#server = app2.server
...
fig8.update_layout(height=270, showlegend=False,template='plotly_dark',width=1200)
...
@app5.callback(Output("download-dataframe-xlsx", "data"),
...
def descargarUsr(n_clicks, usr,hrs,cds):
    ...
    for col in jsondata1:				
        for x in jsondata1[col]:
            dataTr.append([col,x['QUANTITY'],x['F_MOD'],x['DIA'],x['HASTA'],x['LOC'],x['USUARIO'],x['DESDE'],x['FLUJO'],x['HORA_'],x['MINUTO'],x['MIN_HORA'],x['HORA'],x['TRANSACTION_ID'],x['TIPO'],x['CD'],x['ASN_ID'],x['CONTAINER_ID'],x['SHIPMENT_ID'],x['NEW_CONTAINER_ID'],x['JORNADA'],x['TRS'],x['USR']])
...
@app5.callback([Output(component_id='output_user', component_property='children'),
...
def actualizar(usuario,value):
    ...
    fig81.add_trace(go.Scatter(x=df2['F_MOD'], y=df2['LOC'], mode="markers"),
              row=1, col=3)
    fig81.update_layout(height=280,showlegend=False,template='plotly_dark',width=1200)
    return dfSalida['USUARIO'], dfSalida['RUT'],dfSalida['NOMBRE'],c, fig81
@app5.callback([Output(component_id='output_container', component_property='children'),
...
def upgrade_graph(CDslctd,value):
    ...
    if (CDslctd=="ALL"):
        df2 = dfTr.copy()
        figO = px.scatter(df2, x="F_MOD", y="USUARIO",title="Gráfico por tipo de transacción",color='TIPO',height=1400,width=1200,template='plotly_dark')
        figA = px.scatter(df2, x="F_MOD", y="USUARIO",title="Gráfico por zona",color='LOC', height=1400,width=1200,template='plotly_dark')
        figA = figA.update_yaxes(autorange="reversed")
    elif  CDslctd=="CD2":
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por Tipo",color='TIPO', height=1400,width=1200 ,template='plotly_dark')
        figA = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por zona",color='LOC', height=1400,width=1200 ,template='plotly_dark')
        figA = figA.update_yaxes(autorange="reversed")
    elif CDslctd=="CD1":
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por Tipo",color='TIPO', height=1400,width=1200 ,template='plotly_dark')
        ...
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", color="TIPO",title="TR por Tipo", height=1400,width=1200 ,template='plotly_dark')
        figA = px.scatter(df2, x="F_MOD", y="USUARIO", color="LOC",title="TR por zona", height=1400,width=1200,template='plotly_dark')
        figA = figA.update_yaxes(autorange="reversed")
    df2=df2
