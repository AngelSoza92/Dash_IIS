...
app2 = dash.Dash(__name__, title="Dashboard_100",requests_pathname_prefix='/app2/',external_scripts=external_scripts)
#server = app2.server
...
def upgrade_graph(CDslctd,value):
    ...
    yy = pd.read_json('http://10.107.226.241/tracking_labor_hr?horas={}'.format(value))
    csUsr=[]
    cUsrCD1=[]
    cUsrCD2=[]
    cUsrDD=[]
    ...
    if (CDslctd=="ALL"):
        df2 = dfTr.copy()
        figO = px.scatter(df2, x="F_MOD", y="USUARIO",title="Gráfico por tipo de transacción",color='TIPO',height=alto*20,width=1200)
        figA = px.scatter(df2, x="F_MOD", y="USUARIO",title="Gráfico por zona",color='LOC', height=alto*20,width=1200)
        figA = figA.update_yaxes(autorange="reversed")
        print(altoCD2)
    elif  CDslctd=="CD2":
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por Tipo",color='TIPO', height=altoCD2*20,width=1200 )
        figA = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por zona",color='LOC', height=altoCD2*20,width=1200 )
        figA = figA.update_yaxes(autorange="reversed")
        print(altoCD2)
    elif CDslctd=="CD1":
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por Tipo",color='TIPO', height=altoCD1*20,width=1200 )
        figA = px.scatter(df2, x="F_MOD", y="USUARIO", title="TR por zona",color='LOC', height=altoCD1*20,width=1200 )
        figA = figA.update_yaxes(autorange="reversed")
        print(altoCD1)
    else:
        df2 = dfTr[dfTr["CD"]==CDslctd]
        figO = px.scatter(df2, x="F_MOD", y="USUARIO", color="TIPO",title="TR por Tipo", height=altoDD*20,width=1200 )
        figA = px.scatter(df2, x="F_MOD", y="USUARIO", color="LOC",title="TR por zona", height=altoDD*20,width=1200)
        figA = figA.update_yaxes(autorange="reversed")
        print(altoDD)
    df2=df2
