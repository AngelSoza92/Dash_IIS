...
app2 = dash.Dash(__name__, title="Dashboard",external_scripts=external_scripts,requests_pathname_prefix='/app2/',external_stylesheets=[dbc.themes.SLATE, dbc.icons.FONT_AWESOME])
...
def upgrade_graph(CDslctd,value):
    ...
    if (CDslctd=="ALL"):
        ...
        figA = px.scatter(df2, x="F_MOD", y="USR",title="Gráfico por zona",color='LOC', height=1400,width=1200,template='plotly_dark')
    elif  CDslctd=="CD2":
        ...
        figA = px.scatter(df2, x="F_MOD", y="USR", title="TR por zona",color='LOC', height=1400,width=1200 ,template='plotly_dark')
        ...
        figA = px.scatter(df2, x="F_MOD", y="USR", color="LOC",title="TR por zona", height=1400,width=1200,template='plotly_dark')
        figA = figA.update_yaxes(autorange="reversed")
    df2=df2
