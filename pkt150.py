...
appkt150 = dash.Dash(__name__,title="PKT", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/pkt150/')

PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"


contenidox = html.Div(id='page-content',children=[])
appkt150.layout = html.Div([dcc.Location(id="url"), contenidox])

@appkt150.callback([Output("table1","data"),Output("table1","columns"),Output("graph1","figure")],[Input('dFecha','value'),Input('dEstado','value'),Input('dOrdenes','value')])
def updaTable(ifechas, iestados, iordenes):
    dff=get_df5()
    dfa = dff[(dff.FECHA_SALIDA.isin(ifechas)) & (dff.ESTADO.isin(iestados)) & (dff.TIPO_ORDEN.isin(iordenes))]
...
@appkt150.callback([Output("output-table","children"),Input("table1", "active_cell"), State('dFecha','value'),State('dEstado','value'),State('dOrdenes','value')])
def cell_clicked(active_cell, fechaj, estadoj, ordenj):
    dff=get_df5()
    if active_cell is None:
        return no_update
    dfb = dff[(dff.FECHA_SALIDA.isin(fechaj)) & (dff.ESTADO.isin(estadoj)) & (dff.TIPO_ORDEN.isin(ordenj)) ]
    row = active_cell["row"]
...
@appkt150.callback(Output("page-content", "children"), Input("url", "pathname"))
def render_page_content(pathname):
    ...
    [
        html.Div(
            [
                html.H3("Ordenes local 150", style={"textAlign":"center"}),
                html.Div([
                html.P('Fechas, estados'), 
                dcc.Dropdown(
                etas,
                etas,multi=True,placeholder="Fechas...",searchable=True,id='dFecha',style={'maxHeight': '60px', 'overflowY': 'auto'}
                ),
                dcc.Dropdown(
                estados,
...
if __name__ == "__main__":
    appkt150.run_server(debug=True)