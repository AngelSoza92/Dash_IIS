...
appkt100 = dash.Dash(__name__,title="PKT", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/pkt100/')

PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"

contenidox = html.Div(id='page-content', children=[])
appkt100.layout = html.Div([dcc.Location(id="url"),contenidox])

@appkt100.callback([Output("table1","data"),Output("table1","columns"),Output("graph1","figure")],[Input('dFecha','value'),Input('dEstado','value'),Input('dOrdenes','value'),Input('dDestinos','value')])
...
@appkt100.callback([Output("output-table","children"),Input("table1", "active_cell"), State('dFecha','value'),State('dEstado','value'),State('dOrdenes','value'),State('dDestinos','value')])
...
@appkt100.callback(Output("page-content", "children"), Input("url", "pathname"))
...
if __name__ == "__main__":
    appkt100.run_server(debug=True)