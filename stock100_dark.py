...
appst100 = dash.Dash(__name__,title="Stock", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/stock100/')

PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"

contenidox = html.Div(id='page-content',style={"color":"white",'background-color':'black'},children=[])
appst100.layout = html.Div([dcc.Location(id="url"),contenidox])
@appst100.callback([Output("table1","data"),Output("table1","columns"),Output("graph1","figure")],[Input('dCds','value'),Input('dZonas','value'),Input('dDeptos','value')])
...
@appst100.callback([Output("output-table","children"),Input("table1", "active_cell"), State('dCds','value'),State('dZonas','value'),State('dDeptos','value')])
...
@appst100.callback(Output("page-content", "children"), Input("url", "pathname"))
...
if __name__ == "__main__":
    appst100.run_server(debug=True)