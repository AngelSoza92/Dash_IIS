...
from conn3 import get_df2
import dash_bootstrap_components as dbc
import numpy as np
import dash
from plotly.subplots import make_subplots

appa12 = dash.Dash(__name__,title="Antiguedad",external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/antiguedad12/')

PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"


contenidox = html.Div(id='page-content', children=[ ])
appa12.layout = html.Div([dcc.Location(id="url"), contenidox])

@appa12.callback(Output("page-content", "children"), Input("url", "pathname"))
def render_page_content(pathname):
    ...
    fig8 = make_subplots(
        ...
        specs=[
            [{"type": "domain"}, {'type':'xy'}]],
        subplot_titles=("origen ASN","CD")
    )

    fig8.add_trace(go.Pie(labels=origenes, values=dfrespon[origenes], name="Origen ASN"), 1, 1)
    fig8.add_trace(go.Bar(x=whses, y=dfwhses[whses], name='CDs'),1,2)
    content = html.Div(
    [
        html.Div(
            [
                html.H3("LPNs reserva", style={"textAlign":"center"}),
                html.Div([
                html.P('ORIGEN ASN, WHSE'),
                dcc.Dropdown(
                origenes,
...
@appa12.callback([Output("table","data"),Output("table","columns"),Output("table","page_size"),Output("graph1","figure")], [Input('dORIGEN','value'), Input('dWHSE','value')])
def upTable(v_origen,v_status):
    dff=get_df2()
    dfa = dff[(dff.ORIGEN_ASN.isin(v_origen)) & (dff.WHSE.isin(v_status))]
...
@appa12.callback(
    Output("output-graph", "children"), Input("table", "active_cell"),[State('dORIGEN','value'), State('dWHSE','value')]
)
