...
appsdt = dash.Dash(__name__,title="SameDay", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/samedaytodes/')
PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"

contenidox = html.Div(id='page-content',children=[])
appsdt.layout = html.Div([dcc.Location(id="url"), contenidox])

@appsdt.callback(Output("page-content", "children"), Input("url", "pathname"))
...
@appsdt.callback([Output("table1","data"),Output("table1","columns"),Output("graph1","figure"),Output("graph2","figure")],[Input('dLocal','value'),Input('dCumplimiento','value'),Input('dMeses','value'),Input('dReciente','value'),Input('dFlujo','value')])
...
@appsdt.callback([Output("output-table","children"),Input("table1", "active_cell"), State('dLocal','value'),State('dCumplimiento','value'),State('dMeses','value'),State('dReciente','value'),State('dFlujo','value')])
