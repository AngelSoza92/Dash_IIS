...
from conn2 import get_df7
import dash_bootstrap_components as dbc
import numpy as np
import dash


#appc = dash.Dash(__name__,title="CLU", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME])
appc150 = dash.Dash(__name__,title="CLU", external_stylesheets=[dbc.themes.MORPH, dbc.icons.FONT_AWESOME],requests_pathname_prefix='/clu150/')
PLOTLY_LOGO = "http://10.107.226.241/assets/caja.png"



contenidox = html.Div(id='page-content',children=[])
appc150.layout = html.Div([dcc.Location(id="url"), contenidox])

@appc150.callback(Output("page-content", "children"), Input("url", "pathname"))
    ...
    [
        html.Div(
            [
                ...
                dcc.Dropdown(
                olas,
                olas,multi=True,placeholder="OLA...",searchable=True,id='dOla',style={'maxHeight': '60px', 'overflowY': 'auto'}
                ),
                html.P('RETIRA'), 
                dcc.Dropdown(
                retiras,
...
@appc150.callback([Output("table","data"),Output("table","columns"),Output("table","page_size")], [Input('radioGaga','value'),Input('dCD','value'), Input('dFecha','value'), Input('dTipood','value'), Input('dFlujo','value'),Input('dTipoola','value'),Input('dRetira','value'),Input('dEspecial','value'),Input('dOla','value')])
